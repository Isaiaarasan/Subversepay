...
import { OverviewMetrics, PendingApproval } from "@/app/(dashboard)/dashboard/services/overview.service";
...
const SuperAdminClient: React.FC<SuperAdminClientProps> = ({ initialMetrics, initialPendingApprovals }) => {
    const dispatch = useAppDispatch();
    // We can still use Redux for UI state like timeRange, but use props for data
    const { timeRange } = useAppSelector((state) => state.overview); // Only use UI state from store

    // Convert service metrics object to array for display
    const statsArray: MetricCardProps[] = [
        initialMetrics.merchants,
        initialMetrics.managers,
        initialMetrics.customers,
        initialMetrics.tpv
    ];
                        ...
                        >
                            {["This Month", "Last Month", "This Quarter"].map((range) => (
                                <DropdownMenuItem
                                    ...
                                    className={`flex items-center gap-2 cursor-pointer rounded-lg text-xs font-medium px-3 py-2 transition-colors ${timeRange === range
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="bg-white/80 backdrop-blur-xl p-5 rounded-2xl border border-white/60 dark:border-gray-800 shadow-lg shadow-slate-200/50 dark:shadow-none hover:shadow-xl transition-all duration-300 dark:bg-gray-900/80">
                    ...
                    <div className="space-y-3">
                        {initialPendingApprovals.map((item, i) => (
